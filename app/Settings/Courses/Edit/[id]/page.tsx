...
export default function EditCoursePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        (allCourses || []).forEach((c: Course) => { codeToId[c.code] = c.id; });
        const prereqIds = Array.isArray(current.prerequisites) ? 
          current.prerequisites.map((p: any) => codeToId[String(p)] || String(p)).filter(Boolean) : [];
          
        // Set department and degree if available in the course data
        let initialDepartmentId = 0;
        let initialDegreeId = 0;
        
        if (current.departmentId) {
          initialDepartmentId = Number(current.departmentId);
          setSelectedDepartment(initialDepartmentId);
          // Trigger department change to load degrees
          await handleDepartmentChange(initialDepartmentId);
          
          if (current.degreeId) {
            initialDegreeId = Number(current.degreeId);
            setSelectedDegree(initialDegreeId);
            // Trigger degree change to load tracks
            await handleDegreeChange(initialDegreeId);
          }
        }
        ...
        reset({
          ...
          departmentId: initialDepartmentId || undefined,
          degreeId: initialDegreeId || undefined,
          msarId: current.msarId ? Number(current.msarId) : undefined,
  ...
  const filteredTracks = useMemo(() => {
    return tracks; // Currently not filtering, but allows for future filtering by degree or department
  }, [tracks]);
