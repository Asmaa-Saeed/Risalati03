...
export default function AddTrackModal({ isOpen, onClose, onSubmit, loading = false }: AddTrackModalProps) {
  ...
  const {
    ...
    defaultValues: { degreeId: 0, departmentId: 0 },
  });
  ...
  const handleDepartmentChange = (deptId: number) => {
    setValue("degreeId", 0); // reset degree if department changes
    if (deptId) {
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center z-50 p-4 transition-all duration-300 ${
      ...
      <div className={`bg-white rounded-xl shadow-xl max-w-2xl w-full relative z-10 overflow-y-auto transform transition-all duration-300 ${
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="p-6 space-y-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              ...
              >
                ...
                {departments.map((dept) => (
                  <option key={dept.id} value={dept.id}>{dept.value}</option>
                ))}
              </select>
            ...
            {/* الدرجة العلمية */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">الدرجة العلمية <span className="text-red-500">*</span></label>
              <select
              ...
              >
                ...
                {degrees.map((deg) => (
                  <option key={deg.id} value={deg.id}>{deg.value}</option>
                ))}
              </select>
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-gray-200">
            <button type="button" onClick={handleClose} className="px-6 py-3 text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50">
              إلغاء
            </button>
            <button
              type="submit"
